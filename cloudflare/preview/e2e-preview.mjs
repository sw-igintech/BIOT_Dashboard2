#!/usr/bin/env node
// End-to-end UI validation of the dashboard against the REAL Cloudflare staging Worker.
//
// Drives a real Chromium browser through the preview build (serve-preview.mjs), which loads
// the unmodified production dashboard.js/dashboard.css but points window.DASHBOARD_CONFIG at
// the Cloudflare staging Worker. Exercises login, refresh, logout, dashboard load, table,
// search, filter chips, date filter, charts, metrics, device modal + settings fetch, and
// manufacturer scope (org + distributor). Asserts ALL backend traffic hits the Cloudflare
// host (never Supabase) and that there are no uncaught page errors.
//
// Requires Playwright + a Chromium build available. Creds + URLs via env:
//   BIOT_USERNAME, BIOT_PASSWORD   (sourced from claude/biot_credentials.env)
//   PREVIEW_URL   (default http://localhost:8788)
//   STAGING_HOST  (default biot-dashboard-staging.sw-590.workers.dev)
//   E2E_EMAIL / E2E_PASSWORD optional overrides for the login role under test.

import { chromium } from "playwright";

const PREVIEW_URL = process.env.PREVIEW_URL || "http://localhost:8788";
const STAGING_HOST = process.env.STAGING_HOST || "biot-dashboard-staging.sw-590.workers.dev";
...
// Prefer a system Chrome (CHROME_BIN or channel:"chrome") so the run does not depend on a
// specific Playwright-bundled Chromium build being present in the cache.
...
const backendHits = { staging: 0, supabase: 0, other: [] };
const actionsSeen = new Set();
page.on("request", (req) => {
  const u = req.url();
  if (u.includes(STAGING_HOST)) { backendHits.staging++; const a = new URL(u).searchParams.get("action"); if (a) actionsSeen.add(a); }
...
try {
  ...
  const banner = await page.locator("text=PREVIEW — Cloudflare staging").count();
  ...
  await Promise.all([
    page.waitForResponse((r) => r.url().includes(STAGING_HOST) && r.status() >= 200, { timeout: 30000 }).catch(() => {}),
    page.click("#loginBtn"),
  ]);
  await page.waitForSelector("#loginError:not(.hidden)", { timeout: 15000 });
  const loginErrText = (await page.locator("#loginError").textContent())?.trim();
  ok("error handling: bad login shows error", JSON.stringify(loginErrText));
  ...
  // ---- dashboard initial load: table + count -------------------------------
  ...
  const countBadge = (await page.locator("#machinesCount").textContent())?.trim();
  ok("machine table renders", `rows=${rowCount} countBadge=${countBadge}`);

  // ---- charts present (4 canvases sized) -----------------------------------
  ...
  // ---- metrics populated (connection/glove/sanitizer/operational) ----------
  ...
  const metricsOk = metrics.every((m) => m.length > 0);
  metricsOk ? ok("metric widgets populated", metrics.join(" | ")) : warn("some metric widgets empty", metrics.join(" | "));

  // ---- glove metrics specifically (the Stage-3 subrequest-cap fix) ---------
  const gloveText = (await page.locator("#gloveMetrics").textContent() || "").replace(/\s+/g, " ").trim();
  /[1-9]/.test(gloveText) ? ok("glove metrics non-zero (subrequest fix holds)", gloveText.slice(0, 60))
    : warn("glove metrics appear zero", gloveText.slice(0, 60));
  ...
  if (firstId) {
    ...
    (filtered <= rowCount && allMatch) ? ok("search filter", `"${frag}" → ${filtered}/${rowCount} rows, all match`)
      : fail("search filter wrong", `filtered=${filtered} allMatch=${allMatch}`);
    await page.fill("#machineSearch", "");
  ...
  for (const f of ["all", "connected", "disconnected"]) {
    ...
    const active = await page.getAttribute(`.filter-chip[data-filter="${f}"]`, "class");
    if (!active.includes("filter-chip--active")) fail("filter chip active state", f);
  }
  ok("filter chips switch", JSON.stringify(chipCounts));
  await page.click('.filter-chip[data-filter="all"]');
  await page.waitForTimeout(200);

  // ---- date/time filter → Apply triggers scoped reload ---------------------
  ...
  // ---- scope selector (manufacturer): orgs + distributors -----------------
  ...
  if (scopeVisible) {
    ...
    const orgOpt = await page.evaluate(() => Array.from(document.querySelectorAll("#organizationSelect option")).find((o) => o.value.startsWith("org:"))?.value || "");
    const distOpt = await page.evaluate(() => Array.from(document.querySelectorAll("#organizationSelect option")).find((o) => o.value.startsWith("dist:"))?.value || "");
    ok("scope selector visible (manufacturer)", `optgroups=[${groups.join(",")}] sampleOrg=${orgOpt ? "yes" : "no"} sampleDist=${distOpt ? "yes" : "no"}`);

    if (orgOpt) {
      const [r] = await Promise.all([ waitDash(), page.selectOption("#organizationSelect", orgOpt) ]);
      await page.waitForTimeout(400);
      const k = await page.evaluate(() => window?.localStorage && document.querySelector("#machinesCount")?.textContent);
      ok("scope: select organization reloads", `${orgOpt} → HTTP ${r.status()} count=${k}`);
    } else warn("scope org test skipped", "no org option");

    if (distOpt) {
      const [r] = await Promise.all([ waitDash(), page.selectOption("#organizationSelect", distOpt) ]);
      await page.waitForTimeout(400);
      ok("scope: select distributor reloads", `${distOpt} → HTTP ${r.status()}`);
    } else warn("scope dist test skipped", "no dist option");

    // back to all
    await Promise.all([ waitDash(), page.selectOption("#organizationSelect", "all") ]).catch(() => {});
  } else {
    warn("scope selector hidden", "viewer is non-manufacturer (org/distributor role)");
  }

  // ---- device modal + settings fetch --------------------------------------
  await page.click('.filter-chip[data-filter="all"]');
  await page.waitForTimeout(300);
  await page.click("#machinesTableBody tr:first-child");
  await page.waitForSelector("#deviceDetailModal:not(.hidden)", { timeout: 10000 });
  const modalId = (await page.locator("#detailDeviceId").textContent())?.trim();
  const connStatus = (await page.locator("#detailConnStatus").textContent())?.trim();
  ok("device modal opens (Status tab)", `id=${modalId} conn=${connStatus}`);

  // settings tab → triggers entity fetch
  ...
  else if (!entityResp) warn("settings entity fetch not observed", `swVersion="${swVer}" (device may lack current_settings2)`);
  ...
  // ---- refresh-token flow: corrupt access token, keep refresh token --------
  await page.evaluate(() => {
    const rt = localStorage.getItem("auth_refresh_token");
    if (rt) localStorage.setItem("auth_token", "expired.invalid.token");
  });
  ...
  const cleared = await page.evaluate(() => !localStorage.getItem("auth_token"));
  cleared ? ok("logout returns to login + clears token") : fail("logout did not clear token");

  // ---- network + error guards ----------------------------------------------
  backendHits.supabase === 0 ? ok("ALL backend traffic → Cloudflare staging", `stagingHits=${backendHits.staging} supabaseHits=${backendHits.supabase} actions=[${[...actionsSeen].join(",")}]`)
