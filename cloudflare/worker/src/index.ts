// biot-dashboard — Cloudflare Worker (parallel runtime)
//
// PARALLEL MIGRATION TARGET. This Worker is a faithful, behavior-compatible port
// of the active Supabase Edge Function at supabase/functions/biot-dashboard/index.ts.
// It is NOT yet wired to production: the frontend still points at Supabase. This
// file exists so the Cloudflare runtime can be deployed to a preview/staging URL
// and validated against the same request/response contract before any cutover.
//
// Pure BIOT proxy. No database. No caching. No sync.
// Frontend → this Worker → BIOT APIs → Frontend.
//
// Required binding (wrangler.toml [vars] or `wrangler secret put`):
//   BIOT_BASE_URL   https://api.dev.igin.biot-med.com
//
// Actions (identical to the Supabase Edge Function):
//   POST { action: "login", username, password }   → { ok, data: { accessToken, refreshToken, userId } }
//   POST { action: "refresh", refreshToken }        → { ok, data: { accessToken, refreshToken } }
//   GET  ?action=dashboard&...  (x-biot-token hdr) → { ok, data: <dashboard payload> }
//   GET  ?action=entity&id=<uuid>  (x-biot-token)  → { ok, data: <generic entity> }
//   GET/POST { action: "health" }                  → { ok, backend, timestamp }

export interface Env {
  // BIOT API base URL. Set as a plaintext var (non-secret host) or a secret.
  BIOT_BASE_URL: string;
}

const CORS_HEADERS: Record<string, string> = {
  "Access-Control-Allow-Origin": "*",
  // Mirrors the Supabase contract: the frontend still sends apikey + authorization
  // (Supabase anon key) on every request. They are ignored here but must be allowed
  // by CORS so the browser preflight passes when pointed at this Worker.
...
// Thrown when BIOT returns 401 so the Worker can propagate it.
...
// ---------------------------------------------------------------------------
// Entry point — Cloudflare Worker fetch handler
// ---------------------------------------------------------------------------
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (req.method === "POST") {
        try { body = await req.json() as Record<string, unknown>; } catch { /* ignore malformed body */ }
      }
      ...
      // ── health ──────────────────────────────────────────────────────────────
      if (action === "health") {
        return ok({ ok: true, backend: "Cloudflare Worker", timestamp: new Date().toISOString() });
      }

      const config: BiotConfig = { baseUrl: getBaseUrl(env) };
      ...
      if (action === "login") {
        ...
        const data = await loginProxy(config, username, password);
      ...
      if (action === "refresh") {
        ...
        const data = await refreshProxy(config, refreshToken);
        return ok({ ok: true, data });
      }
      ...
      if (action === "dashboard") {
        ...
        const data = await buildDashboard(config, params, userToken);
        return ok({ ok: true, data });
      }
...
function getBaseUrl(env: Env): string {
  const baseUrl = env.BIOT_BASE_URL;
  if (!baseUrl) throw new Error("BIOT_BASE_URL is not set.");
  return baseUrl;
}
...
async function loginProxy(config: BiotConfig, username: string, password: string): Promise<Record<string, unknown>> {
...
async function refreshProxy(config: BiotConfig, refreshToken: string): Promise<Record<string, unknown>> {
...
async function buildDashboard(config: BiotConfig, params: Record<string, string>, accessToken: string): Promise<Record<string, unknown>> {
