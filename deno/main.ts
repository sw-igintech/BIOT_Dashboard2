// biot-dashboard — Deno Deploy entrypoint (MIGRATION TARGET, not yet production)
//
// Deno Deploy app entrypoint. This is a faithful, Deno-native copy of the active production
// Supabase Edge Function (supabase/functions/biot-dashboard/index.ts) — same Deno.serve handler,
// same Deno.env config, same request/response contract — plus two proven-safe deltas:
//   1. device_event page size raised to 1000 (EVENT_PAGE_SIZE)
//   2. an explicit User-Agent on BIOT calls
// and the health/meta backend label reports "Deno Deploy".
//
// It is NOT wired to production: the frontend (index.html) still points at Supabase. Deploying
// this only publishes a *.deno.dev URL; the production cutover (pointing the frontend at it) is
// a separate, isolated step. See docs/DENO_MIGRATION.md.
//
// Pure BIOT proxy. No database. No caching. No sync. Frontend → this → BIOT APIs → Frontend.
//
// Required env var (set in the Deno Deploy app settings, NOT committed):
...
//   GET  ?action=entity&id=<uuid>  (x-biot-token)  → single generic entity (device settings)
